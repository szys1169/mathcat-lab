...
export function statementsHtml(object,renderText){
  return mathematicalStatements(object).map(s=>'<section class="wb-statement">'+
    '<div class="wb-statement-heading">'+badge(mathKind(s.math_kind||s.kind||s.type))+(s.title?'<strong>'+renderText(s.title)+'</strong>':'')+
...
export function sessionHtml(p,session,renderText){
  const events=sessionActivity(p,session).filter(e=>e.type==='message.completed'&&e.text?.trim()),notes=rows(p.nodes).filter(n=>n.author===session.id&&n.node_type==='note');
  const labels={'message.completed':'文本回复'};
  return '<div class="wb-session-output"><p class="wb-muted">'+h(roles[session.role]||session.role)+'</p><p>以下展示猫猫已完成的文本回复。</p>'+
    (events.length?events.map(e=>'<article class="wb-entry"><small>'+h(timeLabel(e.at))+' · '+h(labels[e.type]||'公开活动')+(e.activity.stale?' · 历史调用':'')+'</small>'+
      '<div>'+renderText(e.text)+'</div></article>').join(''):empty('这只猫猫暂时还没有完整的文本回复。'))+
...
export function laboratoryHtml(p,renderText){
  const run=latestRun(p),cards=activityCards(p),major=cards.filter(c=>['main','partner'].includes(c.session.role)),minor=cards.filter(c=>!['main','partner','reviewer'].includes(c.session.role));
  const card=c=>{
    ...
    return '<details class="wb-cat-card wb-cat-row '+(c.active?'is-working':'')+'" data-wb-stable="cat-'+h(s.id)+'"><summary><span class="wb-cat-heading"><span class="wb-cat-icon" aria-hidden="true">'+(s.role==='reviewer'?'◎':'♧')+'</span><span><strong>'+h(sessionName(p,s))+'</strong><small>'+h(c.active?'正在工作':label(s.state))+'</small></span></span><span class="wb-cat-preview">'+renderText(goal||'等待登记当前研究目标')+'</span><span class="wb-cat-expand">查看任务</span></summary><div class="wb-cat-expanded"><p class="wb-muted">'+h(c.phase)+'</p>'+
      ...
      (s.focus&&goal!==s.focus?'<p class="wb-muted">研究焦点：'+renderText(s.focus)+'</p>':'')+
      (route?'<p class="wb-muted">方向：'+renderText(route.title||route.objective||route.id)+'</p>':'')+
      ...
      (['main','partner'].includes(s.role)?action('target-advice','提出建议','data-kind="session" data-id="'+h(s.id)+'"'):'')+'</div></div></details>';
  };
  ...
  return '<section class="wb-card wb-laboratory"><div class="wb-section-heading"><div><span class="wb-eyebrow">猫猫实验室</span><h3>现在在做什么</h3></div><div class="wb-actions">'+action('target-advice','建议整体路线','data-kind="project" data-id="'+h(p.id)+'"')+action('reframe','重新梳理目标')+'</div></div>'+
    '<div class="wb-cat-grid">'+(major.length?major.map(card).join(''):empty('研究开始后，领研猫与伙伴猫会在这里显示当前工作。'))+'</div>'+
...
export function questionsHtml(p,renderText){
  ...
  return '<section class="wb-card wb-question-box"><h3>猫猫提问箱 '+badge(questions.length+' 项')+'</h3><p class="wb-question-status">'+(questions.length?'猫猫们需要你的建议':'猫猫们暂时没有疑问')+'</p>'+(questions.length?'<details data-wb-stable="question-box"><summary>查看 '+questions.length+' 条提问</summary>'+questions.map(q=>'<article class="wb-entry">'+renderText(readable(q.body||q.question||q.text))+(q.context||q.reason?'<p class="wb-muted">'+renderText(readable(q.context||q.reason))+'</p>':'')+(canAnswerQuestion(p,q)?action('answer','回答这条问题','data-id="'+h(q.id)+'"'):empty('这条问题保留为记录，当前研究状态不允许通过答复恢复。'))+'</article>').join('')+'</details>':'')+'</section>';
}
...
export function doubtsHtml(p,renderText){
  ...
  return '<section class="wb-card"><div class="wb-section-heading"><h3>开放疑点</h3>'+badge(all.filter(q=>!['resolved','closed'].includes(q.state||q.status)).length+' 项待解决')+'</div><p class="wb-muted">保存具体数学障碍及其处理记录。尚待核对并不代表已证明错误。</p>'+
    (all.length?all.map(q=>'<article class="wb-doubt wb-entry"><div class="wb-section-heading"><h4>'+renderText(q.title||'待解决的问题')+'</h4>'+badge({resolved:'已解决',closed:'已关闭',open:'待解决',working:'正在处理',blocked:'暂时受阻'}[q.state||q.status]||label(q.state||q.status||'open'))+'</div>'+renderText(q.question||q.exact_statement||q.statement||q.description||readable(q.body))+
      (q.conditions?'<p>适用条件：'+renderText(readable(q.conditions))+'</p>':'')+'<p class="wb-muted">影响：'+renderText(readable(q.impact||q.affected_scope)||'尚未单独说明')+' · 负责：'+h(q.owner_session_id?sessionName(p,rows(p.sessions).find(s=>s.id===q.owner_session_id)||{role:'研究猫'}):'未分配')+'</p>'+
...
export function failuresHtml(p,renderText){
  ...
  return '<section class="wb-card"><h3>失败路线与未完成尝试</h3><p class="wb-muted">区分数学方法被否定、暂时受阻与执行中断。结束路线后，已有成果和依据继续保留。</p>'+
    (results.length?results.map(r=>'<article class="wb-entry wb-route-outcome"><div class="wb-section-heading"><h4>'+renderText(r.title||r.objective||'已记录的尝试')+'</h4>'+badge(outcomeLabel(r.outcome||r.kind||r.reason_kind))+'</div>'+renderText(r.statement||r.objective||r.goal||r.description||'')+
      (r.method?'<p><strong>尝试方法</strong> '+renderText(readable(r.method))+'</p>':'')+(r.conditions?'<p><strong>当时条件</strong> '+renderText(readable(r.conditions))+'</p>':'')+
      '<div><strong>没有继续的原因</strong>'+renderText(readable(r.reason||r.conclusion)||'尚未记录具体原因')+'</div>'+
      (r.scope?'<p><strong>结论适用范围</strong> '+renderText(readable(r.scope))+'</p>':'')+
