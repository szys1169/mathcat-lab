...
export function headerHtml(p,renderText=h,codexStatus=null){
  ...
  return '<div data-wb-stable="live-run-heading"><div class="wb-heading"><div><small>MathCat Lab 2.5.0 · 研究白板</small><h2>'+h(short(p.title||p.name||'数学研究',54))+'</h2><p>'+h(s.state)+' · '+h(s.result)+'</p></div><div class="wb-controls">'+action('mode',run?.mode==='collaborative'?'分工由我批准':'自动安排分工')+action('limits','研究设置')+control+'</div></div>'+
