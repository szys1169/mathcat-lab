...
export function headerHtml(p,renderText=h,codexStatus=null){
  ...
  return '<div data-wb-stable="live-run-heading"><div class="wb-heading"><div><small>MathCat Lab 2.5.3 · 研究白板</small><h2>'+h(short(p.title||p.name||'数学研究',54))+'</h2><p>'+h(s.state)+' · '+h(s.result)+'</p></div><div class="wb-controls">'+action('mode',run?.mode==='collaborative'?'分工由我批准':'自动安排分工')+action('limits','研究设置')+control+'</div></div>'+
...
export function overviewHtml(p,model,renderText) {
  ...
  return '<section class="wb-card"><h3>当前研究目标</h3><div>'+renderText(p.problem||'尚无问题正文')+'</div>'+action('revise','提出改题')+
    ...
    (goals.length?goals.map(g=>'<article class="wb-entry"><strong>'+renderText(g.title||g.exact_statement||g.statement||g.id)+'</strong><p>覆盖判断：'+h(g.status==='reviewed_coverage'?'该目标已有有效审查覆盖':g.status==='historical'?'历史问题版本':'尚无有效覆盖记录')+'</p>'+dataDetails('覆盖证据与问题版本',g)+'</article>').join(''):empty('尚无独立的阶段覆盖记录；不从成果数量推断。'))+'</section>'+
    ...
    (questions.length?questions.map(q=>'<article class="wb-entry"><div>'+renderText(readable(q.body||q.question||q.text))+'</div><p>'+renderText(readable(q.context||q.reason))+'</p>'+(canAnswerQuestion(p,q)?action('answer','明确答复','data-id="'+h(q.id)+'"'):empty(run?.state==='paused'?'本輪已暂停；须先明确恢复到待答状态。':'这是保留的问题记录，当前不允许通过答复恢复研究。'))+'</article>').join(''):
...
export function recordsHtml(p,renderText,limit=30){
  ...
  return list('cycles','领研猫研究与安排',c=>'<article class="wb-entry"><small>'+h(timeLabel(c.created_at||c.at||c.started_at))+'</small><strong>'+h(phaseLabel(c.from_phase||c.from))+' → '+h(phaseLabel(c.to_phase||c.phase||c.to))+'</strong><p>'+h(readable(c.reason))+'</p>'+dataDetails('安排与输入截止记录',c)+'</article>')+
    list('routes','研究路线（组织关系）',r=>'<article class="wb-entry"><strong>'+renderText(r.title||r.objective||r.id)+'</strong><p>'+h(label(r.status))+' · '+h(readable(r.restriction||r.description))+'</p>'+action('route',r.status==='blocked'?'解除禁令':'禁止此路线','data-id="'+h(r.id)+'"')+dataDetails('路线及影响范围',r)+'</article>')+
    ...
    list('memory_entries','记忆与尝试',m=>'<article class="wb-entry"><strong>'+renderText(m.title||m.kind||'研究记忆')+'</strong><small>'+h(timeLabel(m.created_at))+'</small><div>'+renderText(readable(m.text||m.body||m.content||m.summary))+'</div><p class="wb-muted">摘要不替代精确原文；是否可复用须核对适用条件和当前来源状态。</p>'+dataDetails('精确来源、条件与原稿',m)+action('summary-feedback','指出记录问题','data-kind="memory" data-id="'+h(m.id)+'"')+'</article>')+
