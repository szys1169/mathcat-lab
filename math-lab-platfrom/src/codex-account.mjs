...
export function createCodexAccount({ config = {}, connect, spawnProcess = spawn, terminateProcess = terminateOwned, now = Date.now, ttlMs = 45_000, timeoutMs = 15_000 } = {}) {
  ...
  async function attempt() {
    ...
    try {
      ...
      await limit(() => client.request("initialize", { clientInfo: { name: "mathcat_account_status", title: "MathCat account status", version: String(config.version || "2.5.0") }, capabilities: { experimentalApi: false } }));
