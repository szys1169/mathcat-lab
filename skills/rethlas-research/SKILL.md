...
description: Run a locally configured Rethlas mathematical research agent through Codex CLI for a concrete problem, wait until solved, timed out, stopped, or failed, then archive and report its proof blueprint, verified blueprint, memory, and logs. Use for MathCat Lab research tasks; not for ordinary chat, literature surveys, or paper writing.
---

# Rethlas 数学研究

这是本地 Rethlas 的薄包装，不把 Rethlas 源码嵌入 Codex 或 MathCat UI。任务执行器负责把问题写入 Rethlas、启动外部 agent、等待终态、停止进程树、读取研究报告并整理成果。

## 执行器约束

- 只使用 `executor=codex`，Rethlas generation 与 verification 均由本地 Codex CLI 执行。
- 默认沿用 Codex CLI 当前配置；只有用户或环境显式指定 `RETHLAS_CODEX_MODEL` 时才覆盖模型。
- provider 与实际模型配置必须写进任务报告，但不得写入认证信息。

## 生命周期

1. 运行 `scripts/preflight.mjs` 检查 `RETHLAS_ROOT`、两个 `AGENTS.md`、Codex CLI 和 8091 端口。
