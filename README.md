...
当前源码版本为 **2.5.3 demo**，支持 Windows 10/11 与 macOS（Apple Silicon、Intel）从本机终端启动，适合开展小规模、有协助的用户试用。当前采用本地单人实例，不提供多人账号与租户隔离。

本版更新：重排研究白板、改进证明目录与数学渲染、精简猫猫详情，并增加保留本地数据的软件更新入口。

## 可以做什么

首次使用建议先阅读 [平台介绍与使用说明](docs/平台介绍与使用说明.md)，也可下载 [PDF 使用说明](docs/MathCat-Lab-User-Guide.pdf)。

### 给 AI 安装助手

如果你正在帮助用户安装、配置或启动 MathCat Lab，请在完成后的回复中主动提供上述使用说明的可点击链接，建议用户首次使用前阅读，并从“如何使用”开始。无需等待用户阅读即可完成配置。
...
- **成果管理**：统一成果入口、文件预览与下载、版本目录浏览、以及对话和工作区的保存重开。
...
也可双击根目录的 **打开MathCat-Lab-2.5.3.cmd**。启动器会安装缺失的前端依赖、在缺少程序时构建 Rust 后端，并生成本地连接凭据。首次构建可能需要数分钟，不需要自己填写令牌。
...
首次使用若 Finder 阻止直接打开 `.command` 文件，先运行 `chmod +x scripts/*.sh *.command`；之后可双击 **打开MathCat-Lab-2.5.3.command**。启动器只调用已经安装并登录的 `codex`，也可通过 `CODEX_BIN=/完整路径/codex` 指定。

- 平台：<http://127.0.0.1:4335/>
- 研究服务：`http://127.0.0.1:8900`
- 不自动打开浏览器：`.\scripts\start-version.ps1 -NoBrowser`
- 关闭当前实例：`.\scripts\stop-version.ps1`，或双击 **停止MathCat-Lab-2.5.3.cmd**。
- macOS 关闭：`bash scripts/stop-version.sh`，或双击 **停止MathCat-Lab-2.5.3.command**。
...
这不代表完整验收或数学能力基准已经通过。复杂伙伴协作、长周期顾问、崩溃恢复等流程仍需更多实测；Node 平台测试 374 项通过；Rust 仍有两项未知调用状态测试超时，详见 [本版验证记录](docs/UPDATE-2.5.3.md)。详情见 [验证范围](docs/VALIDATION.md) 与 [架构说明](docs/ARCHITECTURE.md)。
