...
- 核心对象、具有实质条件的概念和后文反复使用的术语用正式 `definition` 环境；简单记号可以在首次使用处就地定义。
- 标准概念根据目标读者选择“给出定义”或“精确引用”，但不得在已经参与计算后才补定义。
- 元组与分量、向量与标量、环与理想等容易混淆时使用可区分的字体或宏，并在 `style_decisions.md` 固定选择。
- 参数化对象先定义每个参数值下的对象，再书写族上的等式、同构或不变量。
- 不手写“定理 1.1 的十条关系”等易漂移编号；使用标签或精确来源，并在改动环境后重新编译交叉引用。

## 引用纪律（语料验证）

- `closed-sources` 下，载荷引用只来自来源包书目（`refs.bib` 或等价 BibTeX 文件）。用户明确启用主动文献补全时，新增来源只有达到 `admitted` 后才可进入书目与正文。
- 首次载荷使用处引用一次，之后自然叙述。
- 已有结果：在环境前正文写 "The following is Theorem 3 of \cite{key}" 或 "by a theorem of X \citep{key}"；环境本身是否带标题遵循本文件前述规则，绝不把已有结果写成原创。
- 综述性引用也必须真实、相关且可解析；不得借“综述性提及”绕过文献核验。
...
- [ ] 每个定理/引理有来源（原创或归因）
- [ ] 假设/记号在首次使用前定义
- [ ] 非标准映射给出定义域、陪域与公式；相对对象注明底环/基域
- [ ] 元组与分量、同名字母的不同对象在字体或命名上可区分
- [ ] 无悬空 `\ref`、每个 `\cite` 可解析
